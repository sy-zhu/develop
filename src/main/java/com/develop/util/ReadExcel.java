				...
				if (null != cell) {
					if (c == 0) {
						...
						if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
							...
							map.put("id", name.substring(0, name.length() - 2 > 0 ? name.length() - 2 : 1));// 名称
						} else {
							map.put("id", cell.getStringCellValue());// 名称
						}
					} else if (c == 1) {
						if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
							String sex = String.valueOf(cell.getNumericCellValue());
							map.put("username", sex.substring(0, sex.length() - 2 > 0 ? sex.length() - 2 : 1));// 性别
						} else {
							map.put("username", cell.getStringCellValue());// 性别
						}
					} else if (c == 2) {
						if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
							String age = String.valueOf(cell.getNumericCellValue());
							map.put("pwd", age.substring(0, age.length() - 2 > 0 ? age.length() - 2 : 1));// 年龄
						} else {
							map.put("pwd", cell.getStringCellValue());// 年龄
						}
					}else if (c == 3) {
						if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
							String age = String.valueOf(cell.getNumericCellValue());
						...
						if (cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC) {
							String age = String.valueOf(cell.getNumericCellValue());
							map.put("addr", age.substring(0, age.length() - 2 > 0 ? age.length() - 2 : 1));// 年龄
						} else {
							map.put("addr", cell.getStringCellValue());// 年龄
						}
					}
				}
			}
